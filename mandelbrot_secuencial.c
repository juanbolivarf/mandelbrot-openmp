...
/*
 * mandelbrot_secuencial.c
 * 
 * Programa secuencial que:
 *   Tarea A: Genera una imagen 8K (7680x4320) del Conjunto de Mandelbrot
 *   Tarea B: Aplica un filtro Gaussiano de radio 7 (kernel 15x15) sobre la imagen
 *
 * Salida: fractal_secuencial.ppm (imagen PPM)
 *
 * Compilación:
...
static Pixel iter_to_color(int iter, double zr, double zi) {
    Pixel p;
    if (iter == MAX_ITER) {
        p.r = p.g = p.b = 0;  /* Interior: negro */
        return p;
    }

    /* Suavizado logarítmico para evitar bandas */
    double log_zn = log(zr * zr + zi * zi) / 2.0;
    double nu = log(log_zn / log(2.0)) / log(2.0);
    double smooth = iter + 1.0 - nu;

    /* Mapeo a tono HSV → RGB */
...
/* ────────────────────────────────────────────────
 * TAREA B: Aplicar filtro de convolución 2D
 * ──────────────────────────────────────────────── */
...
int main(void) {
    ...
    /* Asignar memoria para las imágenes */
    ...
    if (!img_fractal || !img_filtrada) {
        fprintf(stderr, "Error: No se pudo asignar memoria (%.1f MB)\n",
                2.0 * img_size / (1024.0 * 1024.0));
        return EXIT_FAILURE;
    }
    ...
    /* Guardar imagen del fractal sin filtro */
    guardar_ppm("mandelbrot_original.ppm", img_fractal);
    printf("  Imagen original guardada: mandelbrot_original.ppm\n\n");
    ...
    printf("  Imagen filtrada guardada: mandelbrot_filtrado.ppm\n\n");
